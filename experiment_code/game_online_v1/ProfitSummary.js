...
class ProfitSummary extends Phaser.GameObjects.Container
{
    constructor(data)
    {
        let {scene, x,y, lpSolution, player, month} = data;
        month = month -1;
        let ownText = new Phaser.GameObjects.Text(scene, -190,-110, 'Eigene Leistung', {fontSize:17,color: '#000'}).setOrigin(0,0.5);
        // Player Values
        let ownProfitText = new Phaser.GameObjects.Text(scene, ownText.x,ownText.y+30, '', {fontSize:17,color: '#000'}).setOrigin(0,0.5);
        let builtText = new Phaser.GameObjects.Text(scene, ownProfitText.x,ownProfitText.y+30, 'Hergestellt:', {fontSize:17,color: '#000'}).setOrigin(0,0.5);
    ...
    set producedBeds(numberOfBeds){
        this._producedBeds.text = 'Betten: '+ numberOfBeds; // We want to give the texfield also a title.
    }
    ...
    makeFeedback(isFinal=false){
        if (isFinal) // did we finish the year?
        {
            ...
            this.feedbackText.text = 'Gute Arbeit!'; // display this text to the user
