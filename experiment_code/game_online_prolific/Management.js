...
class Management extends Building
{
    ...
    produce(item)
    {
        ...
        if (canAfford.status) // check if the item is affordable
        {
            ...
            this.player.availableWood[this.scene.month] -= item.costs.wood;
            this.player.availableMetal[this.scene.month] -= item.costs.metal;
    ...
    sell()
    {
        let profit = this.item.profit[this.scene.month];
        this.scene.buildSound.play();
        this.player.money += profit;
        this.player.monthlyProfit[this.scene.month] += profit;
        switch (this.item.name) // using the item name to add the produced item to the statistics.
        {
            case Constants.ITEM_NAME_CHAIR:
                this.player.producedItems.chairs[this.scene.month] += 1;
                break;
            
            case Constants.ITEM_NAME_TABLE:
                this.player.producedItems.tables[this.scene.month] += 1;
                break;
            
            case Constants.ITEM_NAME_BED:
                this.player.producedItems.beds[this.scene.month] += 1;
                break;

            case Constants.ITEM_NAME_BOOKCASE:
                this.player.producedItems.bookcases[this.scene.month] += 1;
                break;
        }
        if (Constants.PRODUCE_PARTS)
        {
    ...
    canAfford(item)
    {
        ...
        if (costs.wood > this.player.availableWood[this.scene.month]) return {status: false, reason: 'Not enough wood!'};
        if (costs.metal > this.player.availableMetal[this.scene.month]) return {status: false, reason: 'Not enough metal!'};
        if (costs.hoursA > this.scene.workshopA.availableHours) return {status: false, reason: 'No more time in A!'};
        if (costs.hoursB > this.scene.workshopB.availableHours) return {status: false, reason: 'No more time in B!'};
        if (costs.hoursC > this.scene.workshopC.availableHours) return {status: false, reason: 'No more time in C!'};
        if (costs.hoursD > this.scene.workshopD.availableHours) return {status: false, reason: 'No more time in D!'};
