 ...
 */
export default class Diagram extends Phaser.GameObjects.Container{
    constructor(data){
        let {scene, mainScene, lpProfits, playerProfits, months, x,y, height, width} = data;
    ...
    addPlayerProfit(){
        ...
        this.graphics.beginPath();
        this.graphics.moveTo(this.startX,this.startY);
        for (let i = 0; i < this.mainScene.num_month; i++){
            this.graphics.lineTo(this.scaleWidth(i+1), this.scaleHeight(this.playerProfits[this.mainScene.monthLookUP[i]]));
        }
        this.graphics.strokePath();
    }
    ...
    addOptimalProfit(){
        ...
        this.graphics.moveTo(this.startX,this.startY);
        for (let i = 0; i < this.mainScene.num_month; i++){
            this.graphics.lineTo(this.scaleWidth(i+1), this.scaleHeight(this.lpProfits[this.mainScene.monthLookUP[i]]));
    ...
    addDescription(){
        ...
        this.add(new Phaser.GameObjects.Text(this.scene, this.startX+(Constants.DIAGRAM_WIDTH/2), this.maxY, 'Month', {fontSize:14,color: '#000'}).setOrigin(0,1));
        ...
        this.graphics.strokePath();
        this.add(new Phaser.GameObjects.Text(this.scene, this.maxX+20, this.maxY+20, 'You', {fontSize:14,color: '#000'}));
