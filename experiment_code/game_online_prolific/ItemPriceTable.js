...
export default class ItemPriceTable extends Phaser.GameObjects.Container{
    constructor (data){
        ...
        let goldIconBed = new Phaser.GameObjects.Sprite(scene, textBedNew.x+textBedNew.width, spriteBed.y, 'gold').setOrigin(0,0.5).setScale(2);
        let goldIconBookcase = new Phaser.GameObjects.Sprite(scene, textBookcaseNew.x+textBookcaseNew.width, spriteBookcase.y, 'gold').setOrigin(0,0.5).setScale(2);
        let goldIconTable = new Phaser.GameObjects.Sprite(scene, textTableNew.x+textTableNew.width, spriteTable.y, 'gold').setOrigin(0,0.5).setScale(2);
        let goldIconChair = new Phaser.GameObjects.Sprite(scene, textChairNew.x+textChairNew.width, spriteChair.y, 'gold').setOrigin(0,0.5).setScale(2);
        ...
        let title = new Phaser.GameObjects.Text(scene,-85,-85,  'New prices!', {color: '#000', fontSize: 25});
        super(scene,x,y,[spriteFrame,title,spriteBed,spriteBookcase,spriteTable,spriteChair,textBedOld,textBookcaseOld,textTableOld,textChairOld, textBedNew,textBookcaseNew,textTableNew,textChairNew,arrBed,arrBookcase,arrTable,arrChair, goldIconBed, goldIconBookcase, goldIconTable, goldIconChair]);
        this.scene = scene;
        this.texts = texts;
