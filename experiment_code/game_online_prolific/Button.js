...
class Button extends Phaser.GameObjects.Container
{
    ...
    constructor(scene, x ,y, buttonTexture, buttonTexturePressed, buttonImage, callback, context ,tooltipText=undefined)
    {
        ...
        if (image.texture.key === '__MISSING')
        {
            ...
            image = new Phaser.GameObjects.Text(scene,0,0, buttonImage, {fontSize:17,color: '#fff'}).setOrigin(0.5);
        }
     ...
     *      missing material.
     * @param {number} [distance=30] specifies how far the text should travel to the top if shown.
     * @param {number} [duration=0] specifies how long the message should be displayed
     *                      
     */
    showMessage(text, alert=false, distance=30, duration=500)
    {
        let message = new Phaser.GameObjects.Text(this.scene, this.messageOffsetX,0, '', {fontSize:17,color: '#000'}).setOrigin(0.5,0.5);
        if(alert){message.setColor('#cf3434');}
        message.alpha = 0;
        message.text = text;
        this.add(message);
        ...
        this.scene.tweens.add(
            {
                targets: [message],
                alpha: 1,
                y: message.y -distance,
                duration: duration/1.6,
                ease: Phaser.Math.Easing.Sine.Out,
                onCompleteScope: this,
                onComplete: function()
                    {
                        this.closeMessage(message, duration);
                    }
     ...
     * @param {Number} duration the duration of the fade-out animation.
     */
    closeMessage(message, duration)
    {
    ...
    setTooltip(text)
    {
        ...
        this.tooltipText = new Phaser.GameObjects.Text(this.scene, this.tooltip.x, this.tooltip.y,  text, {fontSize:17,color: '#fff'}).setOrigin(0.5,1);
