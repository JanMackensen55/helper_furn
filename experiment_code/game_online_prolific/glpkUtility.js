import {MODEL, SOLUTION, MAX_MONTHS} from './Constants.js';
...
{
    var rawFile = new XMLHttpRequest(); // request the file
    rawFile.open("GET", file, false); // Create an asynchronous request
    rawFile.onreadystatechange = function () // if file as been read:
    {
        if(rawFile.readyState === 4) // if file is ready
        {
            if(rawFile.status === 200 || rawFile.status == 0) // and if it has been found
            {
                var allText = rawFile.responseText; // collect the text from the request (hence the file)
                callback(allText); // pass it to a callback function.
            }
        }
    }
    rawFile.send(null);
...
function getLP(){
    let linearProblem;
    readTextFile(MODEL, function(text){
        linearProblem = text;
    });
...
function loadSolution(){
    let solution;
    readTextFile(SOLUTION,  function(text){
        solution = text;
    });
 ...
 */
function parseLP(){
    let lpFile = getLP().replace(/[\r]/gm, "").split('\n');

    // Parsing profit values
    let filteredVarsProfit = lpFile.slice(0, lpFile.indexOf('s.t.')).filter(element => {
    ...
    let maxMonths = MAX_MONTHS
    let bedArrProfit = profitNumbers.slice(0, maxMonths);
    let bookcaseArrProfit = profitNumbers.slice(maxMonths, 2*maxMonths);
    let tableArrProfit = profitNumbers.slice(2*maxMonths, 3*maxMonths);
    let chairArrProfit = profitNumbers.slice(3*maxMonths, 4*maxMonths);
    
    // Parsing cost values
    let filteredVarsCosts = lpFile.slice(lpFile.indexOf('s.t.'), lpFile.length).filter(element => {
        return element.startsWith('+');
    }).map(element => element.split(' '));;
    let bedArrCosts = filteredVarsCosts.filter(e => e[1] == 'x1' ).map(e => Number(e[0]));
    let bookcaseArrCosts = filteredVarsCosts.filter(e => e[1] == 'x'+Number(maxMonths+1)).map(e => Number(e[0]));
    let tableArrCosts = filteredVarsCosts.filter(e => e[1] == 'x'+Number(maxMonths*2+1)).map(e => Number(e[0]));
    let chairArrCosts = filteredVarsCosts.filter(e => e[1] == 'x'+Number(maxMonths*3+1)).map(e => Number(e[0]));
    
    // Parsing initial values
    let filteredConstraints = lpFile.filter(e => e.startsWith('<='));
    let materials = filteredConstraints.slice(0, maxMonths*2).map(e => Number(e.split(' ')[1]));
    let wood = [];
    let metal = [];
    ...
    let hours = filteredConstraints.slice(maxMonths*2,maxMonths*2+4).map(e => Number(e.split(' ')[1]));
    // Building the linear problem as an object
    let lp = {
...
export {solveLP,loadSolution, parseLP, readTextFile};