 ...
 * @param {object} data.cards The cards with which the player can produce furniture
 * 
 */
class Player {
    constructor(data){
        let {ui, money, wood, metal, hoursA, hoursB, hoursC, hoursD, cards} = data;
     ...
     * @param {Number} addValue The number, indicating how many more times the card should be produced.
     * @returns {{result: boolean, reason: string}} if the player can afford the item given the costs, the result is true and no reason is provided.
     *   If the player cannot afford the item given the costs. the result is false and the reason is a string, informing the player, why he/she 
     *   cannot afford the desired item.
     */
    previewValues(card, addValue){
        let stash = { ...this.stash };
        let factor = stash[card.title].number + addValue 
        let cardCosts = {
