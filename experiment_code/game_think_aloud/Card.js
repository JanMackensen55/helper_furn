...
class Card extends Phaser.GameObjects.Container {
    constructor(data)
    {
        ...
        let spriteImage = new Phaser.GameObjects.Sprite(scene, 0,0, image).setScale(scalingX, scalingY);
        let titleText = new Phaser.GameObjects.Text(scene,0,0,title, {fontSize: 24*scale, color: '#000'});
        let profitText = new Phaser.GameObjects.Text(scene,-110*scalingX,-30*scalingY, "",{fontSize: 19*scale, color: '#000'});
        let costText = new Phaser.GameObjects.Text(scene,profitText.x,profitText.y+30*scalingY, "",{fontSize: 19*scale, color: '#000'});
        ...
        let durationTextOne = new Phaser.GameObjects.Text(scene, -30*scalingX,25*scalingY,'Dauer in ',{fontSize: 15*scale, color: '#000'});
        let durationTextTwo = new Phaser.GameObjects.Text(scene, -30*scalingX,50*scalingY,'Dauer in ',{fontSize: 15*scale, color: '#000'});
        ...
        super(scene,x,y,[spriteImage, goldIcon, potentialGoldIcon, titleText, costText, profitText, durationTextOne, durationTextTwo, potentialProfit,spriteArrowProfit,spriteWorkshopA,spriteWorkshopC,spriteWorkshopB,spriteWorkshopD, woodIcon,ironIcon]);
        /**
    ...
    set title(newName) {
        ...
        this.titleText.y = -65*Constants.SCALINGY;
    ...
    updateCostTexts(costs){
        ...
        if (costs.wood >= 10){
            this.woodIcon.x = this.woodIcon.x+10*Constants.SCALINGX;
            this.metalIcon.x = this.metalIcon.x+10*Constants.SCALINGX;
        }if (costs.metal >=10){
            this.metalIcon.x=this.metalIcon.x+10*Constants.SCALINGX;
        }
        if (this.branch === 1){
     ...
     * @param {int} factor A multiplier for the profits that should be updated.
     */
   scaleProperties(factor){
  
       let canAfford = this.scene.player.previewValues(this, factor);
